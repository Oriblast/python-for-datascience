...
def maj_b(b2, feature, y2, set_name, l):
    b = 0
    w = [0] * len(y2)
    ...
    for i in range(len(y2)):
        ...
        b += y2[i] - y
        l += 1
    b = b2.iloc[0, 0] - b2.iloc[0, 1] * ((1/len(y2)) * b)
    make_data2(["b", "a"], [b, 0.03], set_name)

def maj_w(w2, feature, y2, set_name, a, l):
    w = w2.copy()
    """il s'agit de la dérivé de j sur w et b"""
        ...
        if j != "set":
            ...
            for i in range(len(y2)):
                if (feature["set"][l] == set_name):
                    ...
                    y = 0
                w.loc[0, j] += (y2[i] - y) * feature[j][l]
                l += 1
    for i in w2.columns:
        w2.loc[0, i] = w2.loc[0, i] - a * ((1/len(y2)) * w[i][0])
    ...
    for i in feature.columns:
        if i != s1:
            for j in range(len(feature)):
                ...
                y3[mini] = y2[j]
                mini += 1
                if mini == mini_batch or j + 1 == len(feature):
                    y3[mini+1] = 555 
                    maj_b(b2, feature, y3[:mini], set_name, j - mini)
                    maj_w(w, feature, y3[:mini], set_name, b2["a"][0], j - mini)
                    mini = 0
...
def rLogistic_train(feature, set_name):
    ...
    y2 = y2 = sigmoïde(feature, data_w, data_b.iloc[0, 0], set_name, data_b)
